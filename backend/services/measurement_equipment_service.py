...
class MeasurementEquipmentService:
    ...
    def create_calibration(
        ...
        data, points = MeasurementEquipmentService._normalise_calibration_payload(
            payload
        )
        ...
        if attachment_id is not None:
            certificate_attachment = (
                MeasurementEquipmentService._lock_certificate_attachment(
                    attachment_id
                )
            )
            MeasurementEquipmentService._validate_staged_certificate_ownership(
                certificate_attachment,
        ...
        try:
            ...
            db.session.flush()
            result = MeasurementEquipmentService._calibration_to_dict(record)
            log_audit(
                actor_id,
                "create_calibration",
                "msa_equipment",
    ...
    def approve_calibration(
        ...
        if requested_attachment_id is not None:
            requested_attachment_id = MeasurementEquipmentService._positive_int(
    ...
    def assert_officially_usable(
        ...
        *,
        ...
        if equipment.status == "pending_review":
            MeasurementEquipmentService._raise_validation(
        ...
        record = MeasurementEquipmentService._latest_approved_calibration(
        ...
        if record.result == "limited_use":
            modes = MeasurementEquipmentService._normalise_applicable_modes(
    ...
    def build_snapshot(
        ...
        *,
        ...
        eligibility = MeasurementEquipmentService.assert_officially_usable(
            equipment_id,
            on_date=on_date,
    ...
    def _normalise_calibration_payload(payload) -> tuple[dict, list[dict]]:
        ...
        calibration_type = MeasurementEquipmentService._required_text(
            payload,
            "calibration_type",
        ...
        result = MeasurementEquipmentService._required_text(
            payload,
            "result",
        ...
        normalised_modes = MeasurementEquipmentService._normalise_applicable_modes(
            modes
        )
        ...
        restriction = MeasurementEquipmentService._optional_text(
        ...
        if attachment_id is not None:
            attachment_id = MeasurementEquipmentService._positive_int(
                attachment_id,
                field="certificate_attachment_id",
            )
        data = {
            "calibration_type": calibration_type,
            "calibration_date": MeasurementEquipmentService._parse_date(
                payload.get("calibration_date"),
                field="calibration_date",
                required=True,
            ),
            "effective_date": MeasurementEquipmentService._parse_date(
                payload.get("effective_date"),
                field="effective_date",
                required=False,
            ),
            "next_due_date": MeasurementEquipmentService._parse_date(
                payload.get("next_due_date"),
                field="next_due_date",
                required=False,
            ),
            "calibration_provider": MeasurementEquipmentService._optional_text(
        ...
        points = [
            MeasurementEquipmentService._normalise_correction_point(point, index)
    ...
    def _normalise_correction_point(payload, index: int) -> dict:
        ...
        for field in _CORRECTION_NUMERIC_FIELDS:
            result[field] = MeasurementEquipmentService._decimal_or_none(
                payload.get(field),
                field=field,
        ...
        for field in ("measurement_mode", "unit"):
            result[field] = MeasurementEquipmentService._optional_text(
                payload.get(field),
                field=field,
    ...
    def _calibration_snapshot(
        ...
        return {
            ...
            "applicable_modes": list(
                MeasurementEquipmentService._normalise_applicable_modes(
                    record.applicable_modes
                )
            ),
            "restriction_conditions": record.restriction_conditions,
