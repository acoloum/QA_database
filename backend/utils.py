...
def parse_spec_nominals(spec: Optional[str]) -> Dict[str, float]:
    ...
    支援三種格式：
      兩段式 '外徑*長度'（棒材，如 '34*3000'）：第二值大於外徑時既不可能是內徑也不可能是
             厚度，直接視為長度，不推斷內徑/厚度。
    ...
    註：方管以「外徑/內徑」兩欄借位表示兩個邊長（如 '35*35*1.6*136'），走四段式路徑，
    兩個邊長皆直接讀取，與現場建公差的方式一致。
    """
    ...
    try:
        ...
        if len(nums) >= 4:
            ...
            result['外徑'] = nums[0]
            val2 = nums[1]
            if len(nums) == 2 and val2 > nums[0]:
                # 棒材 '外徑*長度'（如 34*3000）。沿用內徑/厚度的推斷會得出
                # 內徑 3000、厚度 -1483 這種荒謬值，只要公差檔建了內徑或厚度列就會誤判。
                # 邊長相等的方管（35*35）不落入此分支，維持既有的內徑借位解讀。
                result['長度'] = val2
            elif val2 < (nums[0] / 2):
                result['厚度'] = val2
