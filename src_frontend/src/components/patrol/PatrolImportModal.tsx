...
import { useImportPatrol } from '../../hooks/usePatrol';

interface PatrolImportModalProps {
    show: boolean;
    handleClose: () => void;
    onSuccess: () => void;
}

const PatrolImportModal = ({ show, handleClose, onSuccess }: PatrolImportModalProps) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            setError(errorResponse.response?.data?.error || '匯入失敗');
        }
    };
    ...
    return (
        <Modal show={show} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>匯入巡檢數據</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Alert variant="info">請上傳符合格式的 Excel 檔案。</Alert>
                <Form.Group controlId="patrolImportFile" className="mb-3">
...
export default PatrolImportModal;
