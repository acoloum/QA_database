      ...
      tooltip: {
        callbacks: {
          ...
          label: (item: TooltipItem<'line'>) => {
            ...
            if (item.dataset.label === 'UCL' || item.dataset.label === 'CL' || item.dataset.label === 'LCL') {
              return `${item.dataset.label}：${decimal(Number(item.raw))}`;
            }
            return [
              `x（不符合）：${count.nonconforming}`,
              `n（受檢）：${count.inspected}`,
              `比例：${decimal(count.proportion)}`,
              `CL：${decimal(chart.cl[index])}`,
              `LCL：${decimal(chart.lcl[index])}`,
              `UCL：${decimal(chart.ucl[index])}`,
            ];
          },
        },
      },
