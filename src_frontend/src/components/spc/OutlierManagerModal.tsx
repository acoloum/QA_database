                                ...
                                <tr key={m.識別碼} className={m.排除統計 ? 'table-secondary' : ''}>
                                    ...
                                    <td>
                                        {m.排除統計 && (
                                            <div className="small mb-1">
                                                <strong>目前排除：</strong>{m.排除原因 || '—'}
                                                <div className="text-muted">
                                                    操作者 #{m.排除者ID ?? '—'} · {m.排除時間 ? new Date(m.排除時間).toLocaleString('zh-TW') : '時間未記錄'}
                                                </div>
                                            </div>
                                        )}
                                        <Form.Control size="sm" placeholder={m.排除統計 ? '恢復理由（必填）' : '離群原因（必填）'}
                                            value={reasons[m.識別碼] ?? ''}
                                    ...
                                    <td className="text-center">
                                        <Button size="sm"
                                            ...
                                            disabled={setExclusion.isPending || !(reasons[m.識別碼] ?? '').trim()}
