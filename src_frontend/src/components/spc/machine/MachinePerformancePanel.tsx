...
const MachinePerformancePanel = ({ result }: MachinePerformancePanelProps) => {
  ...
  const targetMet = result.available && !result.preliminary
    && result.pm != null && result.pmk != null && result.pm >= targets.pm && result.pmk >= targets.pmk;
  ...
  return <Card><Card.Body>
    ...
    {targetMet ? <Alert variant="success">正式達標：Pm {value(result.pm)} ≥ {value(targets.pm)}，Pmk {value(result.pmk)} ≥ {value(targets.pmk)}；仍須由具核准權限者完成研究核准。</Alert> : <Alert variant="info">尚未宣告正式達標：必須同時具可用、非初步結果，且 Pm 與 Pmk 分別達到目標。</Alert>}
    <Table responsive size="sm" aria-label="機器績效與證據"><tbody>
      ...
      <tr><th>來源觀測語意</th><td>{sourceLabel(evidence.source_semantics)}</td><th>日期範圍</th><td>{dateSpan}</td></tr>
