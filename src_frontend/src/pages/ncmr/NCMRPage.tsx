                            ...
                            {isLoading ? (
                                ...
                                ncmrList.map((item: NCMR) => (
                                    <tr key={item.id}>
                                        ...
                                        <td><PermissionAction permission="ncmr.view" reasonDisplay="tooltip"><button type="button" className="btn btn-link ncmr-result-link" onClick={() => { setDisposeItem(item); setShowDisposeModal(true); }}>{item.result || '-'}</button></PermissionAction></td>
