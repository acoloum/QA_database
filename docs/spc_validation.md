...
| 穩定性 | 位置圖與變異圖分開執行受控規則，再以兩圖共同結果判定研究穩定性 | 任一圖失控即不得宣告製程穩定 |
...
| 正式失控事件 | 只有持續 SPC 使用 active 界限時建立去重事件，後續以 OCAP 保存 6M、重測、調整、產品處置、責任人與有效性 | 回溯研究只顯示診斷違規，不自動建立 OCAP |
...
研究版本保存：方法版本、程式版本、完整篩選、資料雜湊、規格快照、子組值、分布分析值、來源 ID、排除快照、圖表、逐點界限、兩圖穩定性、分布、時間模型、指標與適用性。送審與核准前會以相同轉接器重算來源雜湊；不一致時回 `409 STUDY_DATA_CHANGED`。

報表以 `study_version_id` 從保存樣本重建。修改現在的來源資料後，由舊版本再次匯出的統計與界限必須不變。
