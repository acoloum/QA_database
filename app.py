...
def apply_rework():
    ...
    try:
        ...
        # 檢查NCMR是否存在且判定結果為重工
        cursor.execute(
            "SELECT 識別碼, 判定結果, 狀態 FROM dbo.不合格品單 WHERE 識別碼 = ?",
            (data.get('NCMR_ID'),)
        )
        ...
        conn.commit()
        return jsonify({"success": True, "rework_id": rework_id})
...
def get_rework_executions():
    ...
    try:
        ...
        for row in cursor.fetchall():
            item = dict(zip(cols, row))
            # 格式化日期
            for date_field in ['開始時間', '預計完成時間', '實際完成時間', '建立時間', '更新時間']:
...
def get_rework_costs():
    ...
    try:
        ...
        sql += " ORDER BY c.成本類型, c.記錄日期 DESC"
...
def get_ncmr_info(ncmr_id):
    ...
    try:
        ...
        # 添加調試輸出
        print(f"NCMR查詢SQL執行完成，ID: {ncmr_id}")
        print(f"查詢結果: {row}")
        
        if not row:
            print(f"未找到NCMR記錄，ID: {ncmr_id}")
...
def add_ncmr():
    ...
    try:
        ...
        sql = """
            ...
            (日期, 來源, 來源單號, 產品資訊, 材質, 廠商, 批號, 不良描述, 不合格數量, 發現人員, 判定結果, 狀態)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        cursor.execute(sql, (
            data.get('日期'),
...
def update_ncmr():
    ...
    try:
        ...
        # 所有可更新的欄位
        updatable = [
            '日期', '來源', '來源單號', '產品資訊', '材質', '廠商', '批號', 
            '不良描述', '不合格數量', '發現人員', '判定結果', '狀態'
        ]
        ...
        if not fields:
            return jsonify({"success": True}) # 無變更
...
def delete_ncmr():
    ...
    try:
        ...
        if cursor.fetchone():
            return jsonify({"error": "無法刪除：此異常單已開立矯正措施 (CAPA)，請先刪除關聯的 CAPA 單。"}), 400
...
def create_capa():
    ...
    try:
        ...
        # 建立矯正單
        cursor.execute("""
            INSERT INTO dbo.異常矯正單 (NCMR_ID, 狀態)
            VALUES (?, '進行中')
        """, (ncmr_id,))
